...
@app.route('/network', methods = ['GET']) #route and list of type of methods that are gonna be used on the route
def get_network_ui():
...
@app.route('/boradcast-transaction', methods=['POST'])
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {'message': 'Blockchain seems to be shroter, block not added'}
        return jsonify(response), 409 
